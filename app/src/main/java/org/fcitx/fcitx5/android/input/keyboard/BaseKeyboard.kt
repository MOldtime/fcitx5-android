...
abstract class BaseKeyboard(
    ...
    private fun createKeyView(def: KeyDef): KeyView {
        return when (def.appearance) {
            ...
            val vibration: ((View, Boolean) -> Unit)? =
                if (hapticFeedback) { view, extraConditions ->
                    if (selection.end > 0 && selection.start > 0 && extraConditions) InputFeedbacks.hapticFeedback(
                        view
                    )
                } else null
            if (def is SpaceKey) {
